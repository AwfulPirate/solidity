	...
	Out << generator(
		[&]() {
			return Whiskers(R"(pragma <string>;)")
				("string", validPragmas[Rand() % validPragmas.size()])
				.render();
		},
...
{
	Out << Whiskers(R"(pragma <mutation>;)")
...
string SolidityMutator::typeName()
{
	...
	case 3:
		if (m_numStructs > 0)
			return "s" + to_string(Rand() % m_numStructs);
		else
...
string SolidityMutator::genRandomUserDefinedTypeName(unsigned _numIds, unsigned _len, char _start, char _end)
{
	...
	vector<string> ids{};
	generate_n(back_inserter(ids), Rand() % _numIds + 1, [&](){ return genRandString(_len, _start, _end); });
...
string SolidityMutator::genRandString(unsigned int _maxLen, char _start, char _end)
{
	uniform_int_distribution<char> dist(_start, _end);
	string result{};
	generate_n(back_inserter(result), Rand() % _maxLen + 1, [&]{ return dist(Rand); });
	return result;
}
...
//				[&]() {
//					Out << Whiskers(R"(interface <id> { function f0() external; })")
...
//			  [&]() {
//				Out << Whiskers(R"(library <id> { function f0() public {} })")
...
antlrcpp::Any SolidityMutator::visitEnumDefinition(SolP::EnumDefinitionContext* _ctx)
{
	...
	Out << Whiskers(R"(enum <id> {<elements>})")
...
{
	Out << Whiskers(R"(contract <id> { function f0() public {} })")
			...
			[&]() {
			  if (contractScope())
			  {
				  if (atLeastOneBaseContract && coinToss())
					  return "C" + to_string(Rand() % m_numContracts);
				  else if (atLeastOneBaseInterface && coinToss())
					  return "I" + to_string(Rand() % m_numInterfaces);
				  else
					  return string{};
			  }
	...
	else
	{
		...
		case 1:
			if ((contractScope() || absContractScope()) && !constructorDefined)
				Out << "constructor() external {}";
			break;
		case 2:
			if (!interfaceScope())
				Out << "function f() public {}";
			else
				Out << "function f() external;";
			break;
		case 3:
			genModifier();
			break;
		case 4:
			if (contractScope() || absContractScope())
				Out << "fallback() external {}";
			else if (interfaceScope())
				Out << "fallback() external;";
			break;
		case 5:
			if (contractScope() || absContractScope())
				Out << "receive() external payable {}";
			else if (interfaceScope())
				Out << "receive() external payable;";
			break;
		case 6:
		...
		case 8:
			if (contractScope() || absContractScope())
				...
				Out << "uint constant x = 1337;";
			break;
		case 9:
			Out << "event e() anonymous;";
			break;
		case 10:
			if (!interfaceScope())
				Out << "using L for *;";
...
antlrcpp::Any SolidityMutator::visitConstructorDefinition(SolP::ConstructorDefinitionContext* _ctx)
{
	...
	if (!constructorDefined)
	{
		...
		Out << constructorDef("args", args)("payable", payable)("visibility", visibility).render();
...
antlrcpp::Any SolidityMutator::visitEventDefinition(SolP::EventDefinitionContext* _ctx)
{
	...
	Out << boost::algorithm::join(addEvents, "");
	return antlrcpp::Any();
}
